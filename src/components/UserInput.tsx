...
//   );
//   const webSocketConn = useAppSelector((state)=>state.WebSocketReducer.value)
//   useEffect(() => {
...
//               // @ts-ignore
//               socket.current.send(JSON.stringify({ body }));
//                 //if new chat then adding recent list on left side
...
//                 })
//               );
//               chat_id = load_data.chat_id  // temp updating chat_id
//             }
//           }
//           if (!load_data.is_stream) {
...
import { setUserMessage } from '@/lib/redux/slice/PromptChatData';
...
const userMessage = useAppSelector((state)=>state.PromptChatReducer.value.userMessage)
  const isSocketOpen = useAppSelector((state)=>state.WebSocketReducer.value.isOpen)
  useEffect(() => {
    if (typeof window != "undefined") {
      let screenWidth = window.innerWidth;
      if (screenWidth > 1500) {
  ...
  async function onSubmit() {
    if(isSocketOpen) dispatch(setSendMessage(true))
     return;
    }
